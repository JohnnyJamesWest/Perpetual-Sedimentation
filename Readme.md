...
For convenience we provide a streamlined graphical user interface (see Figure 5) that allows easier interaction with the perpetual sedimentation device. Six common syringe sizes are stored in the software such that the user can select pump output in the more meaningful µL/min form rather than rpm. The control software can be found here (LINK), and the source code here (LINK).
